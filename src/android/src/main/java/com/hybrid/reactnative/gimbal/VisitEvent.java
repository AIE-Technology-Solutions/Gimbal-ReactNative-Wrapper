...
public class VisitEvent implements Event {
    private static final String EXIT_EVENT_NAME = "com.gimbal.region_exit";
    private static final String ENTER_EVENT_NAME = "com.gimbal.region_enter";
    ...
    public WritableMap getBody() {
        ...
        if (visit.getArrivalTimeInMillis() > 0) {
            visitMap.putString(VISIT_ARRIVAL_TIME_KEY, TimeUnit.MILLISECONDS.toMinutes(millis));
        }

        if (visit.getDepartureTimeInMillis() > 0) {
            visitMap.putString(VISIT_DEPARTURE_TIME_KEY, TimeUnit.MILLISECONDS.toMinutes(millis));
        }
