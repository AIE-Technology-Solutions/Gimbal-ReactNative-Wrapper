...
import GimbalEventEmitter from "./GimbalEventEmitter";

/**
 * @hidden
 */
const GimbalAdapterModule = NativeModules.GimbalAdapterModule;

/**
 * @hidden
 */
const eventEmitter = new GimbalEventEmitter();
...
function convertEventEnum(eventType: RegionEventType): string {
  if (eventType === RegionEventType.Enter) {
    return "com.gimbal.region_enter";
  } else if (eventType === RegionEventType.Exit) {
    return "com.gimbal.region_exit";
  }
...
export const GimbalAdapter = {
  ...
  start(): Promise<boolean> {
    return GimbalAdapterModule.start();
  },
  ...
  isStarted(): Promise<boolean> {
    return GimbalAdapterModule.isStarted();
  },
  ...
  getGdprConsentRequirement(): Promise<ConsentRequirement> {
    return GimbalAdapterModule.getGdprConsentRequirement();
  },
  ...
  setUserConsent(consentType: ConsentType, consentState: ConsentState) {
    GimbalAdapterModule.setUserConsent(consentType, consentState);
  },
  ...
  getUserConsent(consentType: ConsentType): Promise<ConsentState> {
    return GimbalAdapterModule.getUserConsent(consentType);
  },
